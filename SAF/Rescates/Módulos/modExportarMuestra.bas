...
'
'  Flujo:
'  1. Valida que existan datos y números de muestra generados.
'  2. Construye el subuniverso filtrado por período y tipo (PN/PJ).
'     - Usa los mismos filtros Año/Mes/TipoInforme que TamañoPoblacion.
'     - El orden del subuniverso sigue el orden de la tabla Rescates.
'  3. Los números aleatorios de la grilla son posiciones dentro de ese subuniverso.
'  4. Exporta las filas seleccionadas a hojas separadas con nombre
'     "Muestra_Rescates_SAF_PN_MesAA" / "_PJ_MesAA".
' ============================================================
Public Sub ExportarMuestra()
    ...
    If wsC Is Nothing Then
        MsgBox "No existe la hoja 'Rescates'. Importe los datos primero.", vbCritical
        Exit Sub
    End If
    ...
    If lo Is Nothing Or lo.DataBodyRange Is Nothing Then
        MsgBox "La tabla 'Rescates' est" & Chr(225) & " vac" & Chr(237) & "a." & vbCrLf & _
    ...
    cntPN = ExportarTipo(wb, lo, "N", "Muestra_Rescates_SAF_PN" & sufijo, celdaPN)
    cntPJ = ExportarTipo(wb, lo, "J", "Muestra_Rescates_SAF_PJ" & sufijo, celdaPJ)
...
'  como posiciones dentro del subuniverso filtrado por período y tipo.
' ============================================================
Private Function ExportarTipo(wb As Workbook, lo As ListObject, _
    ...
    If tipoCol = 0 Then
        MsgBox "No se encontr" & Chr(243) & " la columna 'TIPOPERSONA'.", _
    ...
    If n = 0 Then
        MsgBox "No hay registros de tipo '" & tipoCod & "' en la tabla Rescates." & vbCrLf & _
    ...
    Dim cF As Long: cF = BuscarFechaOperacionEnLO(loT)
...
' Sufijo de hoja: "_MesAA" si hay un solo mes, "" si hay varios
...
' Versión de BuscarColPorNombre que opera sobre un ListObject ya creado
Private Function BuscarColPorNombreEnLO(lo As ListObject, ParamArray nombres() As Variant) As Long
    Dim i As Long, n As Variant, hNorm As String, nNorm As String
    For i = 1 To lo.ListColumns.Count
        hNorm = Canon(lo.ListColumns(i).name)
        For Each n In nombres
            nNorm = Canon(CStr(n))
            If hNorm = nNorm Or InStr(hNorm, nNorm) > 0 Then
                BuscarColPorNombreEnLO = i: Exit Function
            End If
        Next n
    Next i
    BuscarColPorNombreEnLO = 0
End Function

' Versión de BuscarFechaOperacionEnLO para ListObject (duplicada para autonomía del módulo)
Private Function BuscarFechaOperacionEnLO(lo As ListObject) As Long
    Dim variantes As Variant
    variantes = Array("FECHA OPERACI" & Chr(211) & "N", _
                      "FECHA OPERACION", _
                      "FECHA DE OPERACION", _
                      "FECHA PROCESO", _
                      "FECHAOPERACION")
    Dim v As Variant
    For Each v In variantes
        Dim c As Long: c = ColIdx(lo, CStr(v))
        If c > 0 Then BuscarFechaOperacionEnLO = c: Exit Function
    Next v
    Dim i As Long
    For i = 1 To lo.ListColumns.Count
